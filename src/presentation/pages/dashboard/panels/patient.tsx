...
export default function EpidemicSimulation() {
  ...
  const [graphData, setGraphData] = useState<{ day: number; infected: number; deaths: number; recovered: number }[]>([]);
  const [movementOffset, setMovementOffset] = useState(0);
  const [stats, setStats] = useState({ infected: 0, deaths: 0, recovered: 0 });
  ...
  const sirModel = (currentInfected: number, currentRecovered: number, params: SIRParams) => {
    const N = population;
  ...
  const runSimulation = () => {
    ...
    const history: { day: number; infected: number; deaths: number; recovered: number }[] = [];

    const timer = setInterval(() => {
      const { newInfected, newRecovered, deaths } = sirModel(currentInfected, currentRecovered, params);
      ...
      setPeople(prev => prev.map(p => {
        ...
        const isNewInfection = !p.infected && Math.random() < params.beta;
        const shouldDie = p.infected && Math.random() < params.mortalityRate;
        const shouldRecover = p.infected && Math.random() < params.gamma;

        return {
          ...p,
          infected: isNewInfection || (p.infected && !shouldDie && !shouldRecover),
          dead: shouldDie,
          recovered: shouldRecover,
          daysInfected: p.infected ? p.daysInfected + 1 : p.daysInfected,
  ...
  const FloatingCounter = () => (
    <div
      className="fixed right-6 top-6 bg-white/90 backdrop-blur-sm p-4 rounded-lg shadow-xl z-50"
    >
  ...
  return (
    ...
    <div className="flex bg-solution bg-cover flex-col items-center justify-center min-h-screen p-6">
      <Link className="absolute top-2 left-10" to="/landing">
        <Button isIconOnly color="secondary"><ArrowLeft /></Button>
      </Link>
      ...
      <div className="w-full  max-w-6xl">
        {!showResults ? (
          <div className="flex flex-col md:flex-row gap-6">
            <ScrollShadow className="w-full h-[80vh] overflow-y-auto md:w-1/3 space-y-6">
              <Card className="p-4 bg-opacity-45 backdrop-blur-md border-1 border-slate-50">
                <CardHeader className="text-lg font-semibold">Paramètres de base</CardHeader>
                <CardBody className="space-y-4">
                  <Slider label="Population" value={population} onChange={setPopulation} min={10} max={200} />
                  <Slider label="Infectés initiaux" value={infectedCount} onChange={setInfectedCount} min={1} max={population} />
                  <Slider label="Taux d'infection" value={infectionRate} onChange={setInfectionRate} min={0.1} max={1} step={0.1} />
                  <Slider label="Jours de simulation" value={simulationDays} onChange={setSimulationDays} min={5} max={50} />
                </CardBody>
              </Card>

              <Card className="p-4 bg-opacity-45 backdrop-blur-md border-1 border-slate-50">
                <CardHeader className="text-lg font-semibold">Paramètres avancés</CardHeader>
                <CardBody className="space-y-4">
                  <Slider label="Capacité hospitalière (%)" value={hospitalCapacity} onChange={setHospitalCapacity} min={5} max={100} step={5} />
                  <Slider label="Port de masque (%)" value={maskUsage} onChange={setMaskUsage} min={0} max={100} step={5} />
                </CardBody>
              </Card>

              <Button onClick={runSimulation} isLoading={running} disabled={running} color={"secondary"} className="w-full h-12 text-lg">
                {running ? `Simulation en cours...  ${countdown}s` : "Démarrer la simulation"}
              </Button>
            </ScrollShadow>
            ...
            <div className="w-full md:w-2/3">
              ...
              <Card className="h-[80vh] bg-opacity-45 backdrop-blur-md border-1 border-slate-50">
                  <div className="relative w-full flex mx-32 mt-5 h-full p-4">
                    <AnimatePresence>
                      {people.map(person => {
                        const xOffset = Math.sin(movementOffset / 500 + person.id) * 15;
                        const yOffset = Math.cos(movementOffset / 500 + person.id) * 15;

                        return (
                          <motion.div
                            key={person.id}
                            className={`absolute w-3 h-3 rounded-full ${
                              person.dead ? "bg-black" :
                              person.recovered ? "bg-green-500" :
                              person.infected ? "bg-red-500" : "bg-blue-500"
                            }`}
                            animate={{
                              x: person.x + xOffset,
                              y: person.y + yOffset,
                              scale: person.newInfection ? [1, 1.8, 1] : 1
                            }}
                            transition={{
                              type: "spring",
                              stiffness: 30,
          ...
          <Card className="p-6 bg-opacity-45 backdrop-blur-md border-1 border-slate-50">
            <CardHeader className="text-2xl font-bold text-center">Résultats finaux</CardHeader>
            <CardBody>
              <div className="grid grid-cols-3 gap-4 mb-8">
                <div className="text-center">
                  <div className="text-red-500 text-2xl font-bold">{stats.infected}</div>
                  <div className="text-sm">Infectés</div>
                </div>
                <div className="text-center">
                  <div className="text-green-500 text-2xl font-bold">{stats.recovered}</div>
                  <div className="text-sm">Guéris</div>
                </div>
                <div className="text-center">
                  <div className="text-black text-2xl font-bold">{stats.deaths}</div>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={graphData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="infected" stroke="#ef4444" name="Infectés" />
                  <Line type="monotone" dataKey="recovered" stroke="#22c55e" name="Guéris" />
                  <Line type="monotone" dataKey="deaths" stroke="#000" name="Décès" />
                </LineChart>
              </ResponsiveContainer>

              <div className="mt-6 flex justify-center">
                <Button onClick={() => setShowResults(false)} className="px-8 py-3">
                  Nouvelle simulation
                </Button>
              </div>
              </>
